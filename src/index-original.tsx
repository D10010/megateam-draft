...
app.get('/api/stats-original', async (c) => {
  ...
  try {
    ...
    if (type === 'supernode') {
      ...
      // Default multi-endpoint for general stats (simplified for performance)
      const endpoints = [
        'https://apilist.tronscanapi.com/api/system/status',
        'https://apilist.tronscanapi.com/api/system/tps',
        'https://apilist.tronscanapi.com/api/block/latest'
      ]
      ...
      try {
        const statusRes = await fetch(endpoints[0], { 
          headers: { 'Accept': 'application/json', 'User-Agent': 'MEGATEAM-Website/1.0' }
        })
        if (statusRes.ok) {
          const statusData = await statusRes.json()
          allData.network = { ...allData.network, ...statusData }
        }
      } catch (e) {
        console.warn('Status endpoint failed, using defaults')
      }
    ...
    console.error(`❌ Stats API error for type ${type}:`, error)
    return c.json({ 
      error: 'Unavailable',
      type: type,
      timestamp: Date.now()
