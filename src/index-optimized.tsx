...
app.get('/api/tron/block', async (c) => {
  try {
    const data = await tronFetch('block/latest')
    return c.json({
    ...
    return handleApiError('Block', error, TRON_MOCKS.block, c)
  }
})
...
app.get('/api/tron/transactions', async (c) => {
  try {
    const data = await tronFetch('overview/dailytransactionnum')
    ...
    return handleApiError('Transactions', error, TRON_MOCKS.transactions, c)
  }
})
  ...
  try {
    ...
    try {
      ...
      return c.json({
        ...
        volume24h: TRON_MOCKS.price.volume24h,
        change24h: tronData.usd_24h_change || TRON_MOCKS.price.change24h,
        change30d: TRON_MOCKS.price.change30d,
        change1y: TRON_MOCKS.price.change1y,
        marketCap: tronData.usd_market_cap || TRON_MOCKS.price.marketCap,
        rank: TRON_MOCKS.price.rank,
...
app.get('/api/tron/accounts', async (c) => {
  try {
    const data = await tronFetch('account/list?limit=1')
    return c.json({
      totalAccounts: data.account_number || TRON_MOCKS.accounts.totalAccounts,
      activeDaily: data.last_24h_account_change || TRON_MOCKS.accounts.newAccounts24h,
      timestamp: Date.now()
    })
  } catch (error) {
    return handleApiError('Accounts', error, TRON_MOCKS.accounts, c)
  }
})
...
app.get('/api/tron/witnesses', async (c) => {
  try {
    const data = await tronFetch('vote/witness')
    ...
    return handleApiError('Witnesses', error, {
...
app.get('/api/tron/dashboard', async (c) => {
  try {
    ...
    // Use batch fetch with rate limiting for TRONScan endpoints
    const endpoints = [
      'system/tps',
      'block/latest', 
      'overview/dailytransactionnum',
      'account/list?limit=1'
    ]
    
    const results = await tronBatchFetch(endpoints)
    
    // Process TPS data
    const tpsData = results.tps || {}
    const tps = {
      current: Math.round(tpsData.tps || TRON_MOCKS.tps.current),
      max: tpsData.maxTps || TRON_MOCKS.tps.max,
      timestamp: Date.now()
    }
    
    // Process block data
    const blockData = results.latest || {}
    const block = {
      height: blockData.number || TRON_MOCKS.block.height,
      transactions: blockData.nrOfTrx || TRON_MOCKS.block.transactions,
      hash: blockData.hash || '',
    ...
    const transactionsData = results.dailytransactionnum || {}
    let todayTransactions = TRON_MOCKS.transactions.today
    let transactionChanges = { change24h: TRON_MOCKS.transactions.change24h, change7d: TRON_MOCKS.transactions.change7d }
    
    if (transactionsData.data && transactionsData.data.length > 0) {
      const days = transactionsData.data
      const today = days[0]
      todayTransactions = today.newTransactionSeen || TRON_MOCKS.transactions.today
    ...
    const transactions = {
      ...
      totalTransactions: transactionsData.totalTransaction || 8500000000,
      usdtVolume: TRON_MOCKS.usdt.volume,
      ...transactionChanges
    }
    
    // Process account data
    const accountsData = results.limit || {}
    const accounts = {
      totalAccounts: accountsData.rangeTotal || accountsData.total || TRON_MOCKS.accounts.totalAccounts,
      activeAccounts: Math.round((accountsData.rangeTotal || TRON_MOCKS.accounts.totalAccounts) * 0.02),
      newAccounts24h: TRON_MOCKS.accounts.newAccounts24h
    }
    
    // Fetch price data separately (no rate limit on CoinGecko)
    let price = TRON_MOCKS.price
    try {
      const priceResponse = await externalFetch('https://api.coingecko.com/api/v3/simple/price?ids=tron&vs_currencies=usd&include_24hr_change=true&include_market_cap=true')
      const tronData = priceResponse.tron || {}
      price = {
        price: tronData.usd || TRON_MOCKS.price.price,
        change24h: tronData.usd_24h_change || TRON_MOCKS.price.change24h,
        change30d: TRON_MOCKS.price.change30d,
...
app.get('/api/stats', async (c) => {
  ...
  try {
    ...
    if (endpointMap[type]) {
      const data = await tronFetch(endpointMap[type])
