...
import { TRON_FALLBACKS, DASHBOARD_FALLBACK, STATS_FALLBACK, SUPERNODE_FALLBACK } from './utils/mocks'
import { mapWitnessToLocation, getContinent, getGeographicStats } from './utils/geo'
...
app.get('/api/tron/tps', async (c) => {
  try {
    const data = await tronApiFetch('system/tps', {}, c);
    if (data) {
      return createSuccessResponse({
        current: data.data?.currentTps || data.tps || 0,
...
// OPTIMIZED: Combined API endpoint with parallel fetching and centralized fallbacks
app.get('/api/tron/dashboard', async (c) => {
  try {
    console.log('📊 Fetching TRON dashboard data with parallel requests...')
    
    // Use Promise.all for parallel requests (faster, no artificial delays)
    ...
    console.log('🔄 Processing dashboard data with fallbacks...')

    // Process responses with centralized fallbacks
    const tps = {
      current: tpsData?.data?.currentTps || tpsData?.tps || TRON_FALLBACKS.tps.current,
      max: tpsData?.data?.maxTps || TRON_FALLBACKS.tps.max,
      timestamp: Date.now()
    }
    
    const block = {
      height: blockData?.number || blockData?.height || TRON_FALLBACKS.block.height,
      transactions: blockData?.transactionCount || TRON_FALLBACKS.block.transactions,
      hash: blockData?.hash || '',
      size: blockData?.size || 0,
      timestamp: blockData?.timestamp || Date.now()
    }
    
    // Process transaction data with simplified logic
    const transactions = {
      today: transData?.data?.[0]?.newTransactionSeen || transData?.data?.[0]?.num || TRON_FALLBACKS.transactions.today,
      change24h: TRON_FALLBACKS.transactions.change24h, // Use fallback percentages 
      change7d: TRON_FALLBACKS.transactions.change7d,
      totalTransactions: 8500000000 // Estimated total
    }
    
    // Process price data with fallbacks  
    const tronPrice = priceData?.tron || priceData?.market_data;
    const price = {
      price: tronPrice?.usd || tronPrice?.current_price?.usd || TRON_FALLBACKS.price.price,
      change24h: tronPrice?.usd_24h_change || tronPrice?.price_change_percentage_24h || TRON_FALLBACKS.price.change24h,
      change30d: tronPrice?.price_change_percentage_30d || TRON_FALLBACKS.price.change30d,
      change1y: tronPrice?.price_change_percentage_1y || TRON_FALLBACKS.price.change1y,
      marketCap: tronPrice?.usd_market_cap || tronPrice?.market_cap?.usd || TRON_FALLBACKS.price.marketCap,
      volume24h: tronPrice?.total_volume?.usd || TRON_FALLBACKS.price.volume24h,
    ...
    console.log('✅ Combined dashboard data processed with fallbacks')
...
// MERGED: Enhanced stats API with both original and optimized functionality  
app.get('/api/stats', async (c) => {
  const { type = 'all' } = c.req.query();
  
  try {
    // Handle supernode requests with enhanced mapping
    if (type === 'supernode') {
      const supernodeEndpoints = [
        'vote/witness?limit=100',
        'supernode/list?limit=100', 
        'witness'
      ];
      
      for (const endpoint of supernodeEndpoints) {
        try {
          const data = await tronApiFetch(endpoint, {}, c);
          if (data) {
            const nodes = (data.data || data.witnesses || data).map((node, index) => {
              ...
              return {
                ...
                url: node.url || '',
                votes: node.voteCount || node.realTimeVotes || node.votes || 0,
                isActive: node.isJobs === true || node.active === true || index < 27,
                rank: node.realTimeRanking || node.ranking || index + 1,
                location,
      ...
      // If all endpoints fail, return enhanced fallback 
      return createSuccessResponse({
        type: 'supernode',
        total: SUPERNODE_FALLBACK.length,
        active: SUPERNODE_FALLBACK.filter(n => n.isActive).length,
        data: SUPERNODE_FALLBACK,
        source: 'fallback_data'
      }, c);
    }
    
    // Handle consolidated stats using parallel fetching
    if (type === 'all') {
      console.log('🔄 Fetching consolidated stats with parallel requests...');
      
      const endpoints = {
        system: 'system/status',
        tps: 'system/tps', 
        witness: 'vote/witness?limit=100',
        account: 'account/list?limit=1'
      };
      
      const results = await fetchParallel(endpoints, c);
      
      return createSuccessResponse({
        system: results.system?.data || results.system,
        tps: results.tps?.data || results.tps, 
        supernode: results.witness?.data || results.witness,
        account: results.account?.data || results.account,
        type: 'combined',
    ...
    if (['system', 'asset', 'account'].includes(type)) {
      const data = await tronApiFetch(`${type === 'asset' ? 'asset?asset=TRX' : type === 'system' ? 'system/status' : 'account/list?limit=1'}`, {}, c);
      if (data) {
        return createSuccessResponse({ type, data: data.data || data }, c);
      }
    }
    ...
    return handleApiError('Stats', STATS_FALLBACK, c);
  }
});

// REMOVED: /api/stats-original merged into /api/stats above
// Old endpoint removed to eliminate redundancy
  ...
  try {
    ...
    if (type === 'supernode') {
      // Try multiple TRONScan supernode endpoints with fallbacks
      const supernodeEndpoints = [
        'https://apilist.tronscanapi.com/api/supernode/list?limit=100',
        'https://apilist.tronscanapi.com/api/vote/witness?limit=100',
        'https://apilist.tronscanapi.com/api/witness'
      ]
      
      for (const endpoint of supernodeEndpoints) {
        try {
          console.log(`🔄 Trying supernode endpoint: ${endpoint}`)
          ...
          if (res.ok) {
            ...
            // Process supernode data with enhanced mapping
            const nodes = (data.data || data.witnesses || data).map((node, index) => {
              const name = node.name || node.address || `Validator ${index + 1}`
              const location = mapWitnessToLocation(name, node.url)
              ...
              return {
                ...
                isActive: node.isJobs === true || node.active === true || index < 27,
                rank: node.realTimeRanking || node.ranking || index + 1,
                country: location?.country || node.country || 'Unknown',
                latitude: location?.lat || node.latitude || node.lat || 0,
                longitude: location?.lng || node.longitude || node.lng || 0,
                productivity: node.productivity || 100,
                efficiency: node.efficiency || 100
              }
            })
            
            console.log(`✅ Supernode API success: ${nodes.length} nodes from ${endpoint}`)
      ...
      // If all endpoints fail, return enhanced fallback data
      console.log('📋 All supernode APIs failed, using enhanced fallback data')
      const fallbackSupernodes = [
      ...
      return c.json({
        type: 'supernode',
        total: fallbackSupernodes.length,
        active: fallbackSupernodes.filter(n => n.isActive).length,
        data: fallbackSupernodes,
        timestamp: Date.now(),
        source: 'fallback_data',
        note: 'Using fallback data due to API rate limiting'
      })
      
    } else {
// Removed: old stats-original endpoint code cleaned up
        ...
        <div class="container mx-auto px-6 relative z-10">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 mb-16">
            ...
            <div class="cyber-card p-4 sm:p-6 rounded-xl text-center" data-aos="fade-up" data-aos-delay="100">
              ...
              <div id="live-tps" class="stat-tps text-2xl sm:text-3xl font-black text-white mb-2 min-h-[2rem]">--</div>
            ...
            <div class="cyber-card p-4 sm:p-6 rounded-xl text-center" data-aos="fade-up" data-aos-delay="200">
              ...
              <div id="live-block" class="stat-block text-2xl sm:text-3xl font-black text-white mb-2 min-h-[2rem]">--</div>
            ...
            <div class="cyber-card p-4 sm:p-6 rounded-xl text-center" data-aos="fade-up" data-aos-delay="300">
              ...
              <div id="live-daily-txns" class="stat-txns-24h text-2xl sm:text-3xl font-black text-white mb-2 min-h-[2rem]">--</div>
            ...
            <div class="cyber-card p-4 sm:p-6 rounded-xl text-center" data-aos="fade-up" data-aos-delay="400">
              ...
              <div id="live-trx-price" class="stat-trx-price text-2xl sm:text-3xl font-black text-white mb-2 min-h-[2rem]">--</div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8" data-aos="fade-up" data-aos-delay="500">
            ...
            <div class="cyber-card p-8 rounded-xl">
              ...
              <div class="flex items-center">
                ...
                <span id="live-usdt-volume" class="stat-usdt-volume ml-2 text-tron-light font-semibold">Loading...</span>
              </div>
            </div>
            ...
            <div class="cyber-card p-8 rounded-xl">
              ...
              <div class="flex items-center">
                ...
                <span id="live-total-accounts" class="stat-accounts ml-2 text-tron-light font-semibold">Loading...</span>
              </div>
            </div>
          </div>
        ...
        <div class="container mx-auto px-6 relative z-10">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12" data-aos="fade-up" data-aos-delay="200">
            ...
            <div class="cyber-card p-4 sm:p-6 rounded-xl text-center border border-tron-red/20 bg-gradient-to-br from-tron-black/50 via-tron-dark/30 to-tron-black/50">
              ...
              <div id="total-validators" class="stat-validators text-xl sm:text-2xl font-black text-white min-h-[2rem]">Loading...</div>
            ...
            <div class="cyber-card p-4 sm:p-6 rounded-xl text-center border border-tron-light/20 bg-gradient-to-br from-tron-black/50 via-tron-dark/30 to-tron-black/50">
              ...
              <div id="super-reps-count" class="stat-super-reps text-xl sm:text-2xl font-black text-white min-h-[2rem]">Loading...</div>
            ...
            <div class="cyber-card p-4 sm:p-6 rounded-xl text-center border border-tron-silver/20 bg-gradient-to-br from-tron-black/50 via-tron-dark/30 to-tron-black/50">
              ...
              <div id="continents-count" class="stat-continents text-xl sm:text-2xl font-black text-white min-h-[2rem]">Loading...</div>
            ...
            <div class="cyber-card p-4 sm:p-6 rounded-xl text-center border border-green-400/20 bg-gradient-to-br from-tron-black/50 via-tron-dark/30 to-tron-black/50">
              ...
              <div id="network-health" class="stat-network-health text-xl sm:text-2xl font-black text-green-400 min-h-[2rem]">Healthy</div>
          ...
          <div class="cyber-card p-8 rounded-2xl mb-12" data-aos="fade-up" data-aos-delay="400">
            ...
            {/* Map Container */}
            <div class="relative bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl border border-tron-red/30 overflow-hidden shadow-2xl" style="height: 500px;">
              <div id="tron-world-map" class="w-full h-full z-10 rounded-xl"></div>
            ...
            <div class="cyber-card p-6 rounded-xl">
              ...
              <div class="space-y-2">
                <div class="flex justify-between text-sm">
                  ...
                  <span id="exchange-count" class="stat-exchanges text-blue-400">Loading...</span>
                </div>
                <div class="flex justify-between text-sm">
                  <span class="text-gray-400">Independent:</span>
                  <span id="independent-count" class="stat-independent text-green-400">Loading...</span>
                </div>
              </div>
            </div>
